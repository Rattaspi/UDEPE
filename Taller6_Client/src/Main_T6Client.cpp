...
void LogIn(sf::UdpSocket* socket, std::string user, std::string password);
...
int main() {
	...
	enum ProgramState{LOGIN, MATCH_SELECTION,MATCH};
	...
	while (!end){
		...
		case LOGIN: {
			...
			if (window.isOpen()) {
				...
				while (window.pollEvent(evento)) {
					...
					case sf::Event::MouseButtonPressed:
						if (evento.mouseButton.button == sf::Mouse::Left) {
							...
							if (x >= 480 && x <= (480 + 300)) {
								if (y >= 600 && y <= (600 + 45)) {
									Register(socket, myRegisteredUsername, myRegisteredPassword, myEmail);
								}
							}
					...
					case sf::Event::KeyPressed:
						if (evento.key.code == sf::Keyboard::Escape) {
							...
							case 3:
								...
								break;
							case 4:
								myEmail=myEmail.substr(0, myEmail.size() - 1);
								break;
							default:
								break;
							}
						...
						if (evento.text.unicode >= 32 && evento.text.unicode <= 126 && myUsername.length() < 20) {
							switch (selectedOption) {
								...
								case 3:
									...
									break;
								case 4:
									myEmail += (char)evento.text.unicode;
									break;
								default:
									break;
							}
				...
				sf::Text* emailText = new sf::Text("Email: " + myEmail, font, 20);
